...
const editor = readFileSync(new URL('../webapps/better-notebooks/editor.js', import.meta.url), 'utf8');
const browserSmoke = readFileSync(new URL('../webapps/better-notebooks/browser-smoke.js', import.meta.url), 'utf8');
const requiredMarkup = ['id="cells"', 'id="notebook-tabs"', 'id="run-all"', 'id="notebook-download-menu"', 'id="notebook-actions-menu"', 'id="data-panel-toggle"', 'id="cell-minimap"'];
...
if (editor.includes('linter(() => [])')) throw new Error('An empty CodeMirror linter clears externally supplied diagnostics.');
